...
export default class CheckoutForm extends Component {
  ...
  handleSubmit = async (event) => {
    ...
    if (result.error) {
      ...
      fetch(
        "https://wod-with-faris-backend.herokuapp.com/users/buyclasspasses",
        {
          method: "POST",
          headers: {
        ...
        .then((resp) => resp.json())
        .then((resp) => this.cardPayment(resp.client_secret));
    }
  };
  ...
  cardPayment = async (clientSecret) => {
    ...
    if (result.error) {
      ...
      if (result.paymentIntent.status === "succeeded") {
        ...
        fetch(
          "https://wod-with-faris-backend.herokuapp.com/users/addclasspasses",
          {
            method: "POST",
            headers: {
              Accept: "application/json",
      ...
      <div>
        ...
        >
          <div className="checkout-form">
            ...
            {paymentError && (
              <p className="payment-error">Error: {paymentError}</p>
            )}
            {paymentSuccess ? (
              <h1 className="payment-success">
                Thank you! {this.props.quantity}X Class Passes have been added
                to your account!
              </h1>
            ) : (
              <>
                <div class="product-info">
                  <h3 className="product-title">
                    {quantity === 1
                      ? "1X Class Pass"
                      : quantity + "X Class Passes"}
                  </h3>
                  <h4 className="product-price">Total: CHF {quantity * 15}</h4>
                </div>
